...
def open_path(path: str):
    """Cross-platform helper to open a file/folder with the system's default program."""
    try:
...
class MidasDepth:
    def __init__(self):
        self.model = None
        self.transform = None
        self.device = "cuda" if (CFG.get("use_gpu_if_available") and TORCH_OK and torch.cuda.is_available()) else "cpu"
        if TORCH_OK:
            self._load()
    ...
    def gen_depth(self, img_path: Path, out_path: Path) -> bool:
        if self.model is None:
            ...
            if img is None:
                log(f"Image not found for depth: {img_path}")
                return False
...
def build_mesh(depth_png: Path, export_dir: Path, export_fmt: str):
    blender = Path(CFG.get("blender_path"))
...
class App(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(CFG.get("brand"))
        ...
        self.btn_open.clicked.connect(lambda: open_path(str(OUTPUT)))
        top.addWidget(self.btn_open)

        self.btn_settings = QtWidgets.QPushButton("Edit Settings")
        self.btn_settings.clicked.connect(lambda: open_path(str(CONFIG_FILE)))
