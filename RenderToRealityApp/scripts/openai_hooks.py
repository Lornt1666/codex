"""OpenAI integration for project summaries and status updates."""
try:
    import openai
...
def ask(prompt: str, tone="technical", temperature=0.5):
    """Ask OpenAI a question with specified tone and temperature."""
    if not OPENAI_AVAILABLE:
        raise RuntimeError("openai package not available")
...
def gen_risk_analysis(dataset_summary: str, tone="technical"):
    """Generate a risk analysis from dataset summary."""
    prompt = f"""
...
def summarize_mesh_report(mesh_summary: dict, tone="technical"):
    """Generate a summary of mesh build results."""
    prompt = f"""
